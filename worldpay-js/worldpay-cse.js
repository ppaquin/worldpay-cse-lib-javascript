...
function useForm(arg1, arg2) {
  ...
  if (typeof arg1 === "object") {
    //Form reference passed in
    paymentForm = arg1;
    handleError = arg2;
  } else {
    //Search for form reference
  ...
  if (paymentForm !== null) {
    ...
    throw new Error(
      "Payment form was not supplied, and could not be found in the DOM. Did you forget to add 'data-worldpay=\"payment-form\"' to your form?"
    );
  }
}
...
function doAesEncryption(data, aad, aesKeySize) {
  ...
  var gcmOutput = sjcl.mode.gcm.encrypt(
    blockCipher,
    plainText,
    iv,
    aadBits,
  ...
  var encryptedData = sjcl.bitArray.bitSlice(
    gcmOutput,
    0,
    gcmOutputLength - tagLength
  );
  var tag = sjcl.bitArray.bitSlice(
    gcmOutput,
    gcmOutputLength - tagLength,
    gcmOutputLength
  );
...
function validateCardHolderName(cardHolderName) {
  if (!isEmpty(cardHolderName)) {
    if (evaluateRegex(cardHolderName, "^.{1,80}$")) {
      return 0;
    } else {
      return 402;
    }
